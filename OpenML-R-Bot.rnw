...
The datasets with their specific characteristics can be found in table \ref{tab:data}. 
...
<<datasets, echo = FALSE, results="asis",  fig.pos='htb!' , message = FALSE, fig.lp="fig:">>=
  ...
  tasks = tasks[, c("task.id", "name", "number.of.instances", "number.of.features", "majority.class.size")]
  tasks$majority.class.size = tasks$majority.class.size / tasks$number.of.instances
  colnames(tasks) = c("Task_id", "Name", "nObs", "nFeat", "MajPerc")
  
  load("./data/nr_results.RData")
  
  nr_results[,which(!colnames(nr_results) %in% sort(tasks$Task_id))]
  
  print(xtable(tasks, caption = "Included datasets"), digits = 0, label = "tab:data", include.rownames=FALSE, size = "scriptsize")
@
