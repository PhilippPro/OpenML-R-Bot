...
On the other hand, to the best of our knowledge there seem to be very few papers and projects that provide data for their meta-learning approaches, like we do in this paper. One of them is the paper by \citet{Reif2012ACD} which 
presents a dataset for meta-learning based on 83 datasets, six classification algorithms,  and 49 meta-features. The dataset is not anymore available on the university wegpage as the researcher does not work there anymore. We tried to improve this by uploading our result on figshare which is meant to be a durable and open platform for storing and sharing data. 
...
The code for the bot can be found on GitHub (\url{https://GitHub.com/ja-thomas/OMLbots}). Further algorithms can be easily added by adding an algorithm with specific hyperparameters in the \texttt{R/botSetLearnerParamPairs.R} file. The main function \texttt{runBot} executes the bot with specific settings (number of experiments, temporary file, sample configurations, etc.).
