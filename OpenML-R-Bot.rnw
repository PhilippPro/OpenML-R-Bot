...
\section{Method to create the dataset}
...
Furthermore we choose the classification tasks (datasets) from the OpenML100 
Benchmark suite \citep{Bischl2017} as benchmark datasets, only including datasets without 
missing data and with binary outcome resulting in 39 datasets.
...
<<datasets, echo = FALSE, results=tex,  fig.pos='htb!' , message = FALSE, fig.lp="fig:">>=
  ...
  colnames(tasks) = c("Data_id", "Name", "nObs", "nFeat", "majPerc", "numFeat", "factFeat")
  tasks$majPerc = tasks$majPerc/tasks$nObs
  
  print(xtable(tasks, caption = "Included datasets with meta-data", digits = 0, label = "tab:datasets"), digits = 0, include.rownames=FALSE, size = "scriptsize")
@
...
After having run more than 6 million benchmark experiments the results of the bot are downloaded from \textit{OpenML}. 
...
We experienced that the performance of the surrogate models is high enough with 500000 experiments, so that the runtime for the calculations can be reduced.

The 500000 experiments for each algorithm are chosen by the following procedure: for each algorithm, a threshold $B$ is set (see below) and, if the number of results for a dataset exceeds $B$,  we draw randomly $B$ of the results obtained for this algorithm and this dataset. For each algorithm, the threshold value $B$ is chosen for each algorithm separately to exactly obtain 500000 results for each algorithm. 

For \texttt{kknn} we only executed around 125000 experiments because this 
number of experiments is high enough to cover the hyperparameter space (that only consists of the parameter $k$) appropriately. 
This results in a total of around 2.625 million experiments that are used for our analysis and can be downloaded from the figshare 

\section{Access to the benchmark results}

\section{Discussion of and access to the results}
In total more than 6 millions (XXX) runs of the random bot on different 
server platforms like Azure were executed.
...
\item Alternatively the code for the extraction of the data from the nightly database snapshot of \textit{OpenML} can 
be found here: \url{https://github.com/ja-thomas/OMLbots/blob/master/snapshot_database/database_extraction.R}
\end{itemize}


Weakness: Dimension is too high, e.g. for xgboost. The best regions are not explored enough. 
Maybe add later. 

See the following table for the resulting dataset.

<<tbl_results, echo = FALSE, results=tex,  fig.pos='htb!' , message = FALSE, fig.lp="fig:">>=
  library(OpenML)
  tasks = listOMLTasks(number.of.classes = 2L, number.of.missing.values = 0, 
