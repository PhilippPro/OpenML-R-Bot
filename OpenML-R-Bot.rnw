...
\item Load the dataset from cache or download it from OpenML and cache it.
\item Evaluate the algorithm with the sampled hyperparameters on the selected dataset with 10-fold cross-validation. Consistent cross-validation splits for each dataset are provided by OpenML.
...
hyperparameter space with higher probability \citep[see][]{Rijn2017, Probst20182}.
\end{itemize}
