...
In this paper we describe how experiments with the random bot were executed on OpenML (cite XXX). 
...
be found in table XXX. Some hyperparameters are transformed after they have been drawn, 
...
Benchmarksuite (cite XXX) as benchmark datasets, only including datasets without 
missing data. The datasets with their specific characteristics can be found in table XXX. 

The bot then essentially does the following:
\begin{enumerate}
...
\begin{itemize}
\item The easiest way to access them is to go to the figshare repository (XXX) and
download the .RData or the .csv file. 
\end{itemize}
